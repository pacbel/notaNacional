...
interface TomadoresContainerProps {
  prestadores: Prestador[];
}

export default function TomadoresContainer({ prestadores }: TomadoresContainerProps) {
  ...
  // Campos de filtro para o relatu00f3rio
  const camposFiltro = [
    {
      nome: 'prestadorId',
      label: 'Prestador',
  ...
  const handleImprimir = async () => {
    try {
      ...
      const response = await fetch('/api/relatorios/tomadores-html', {
        method: 'POST',
        headers: {
