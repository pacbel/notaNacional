...
import { buscarServicos } from '@/queries/relatorios/servicosQueries';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = gerarHTMLRelatorio(prestadorComLogo, resultado.servicos, resultado.paginacao, filtros);
    ...
    console.error('Erro ao gerar relatório HTML de serviços:', error);
...
function gerarHTMLRelatorio(prestador: any, servicos: any[], paginacao: any, filtros: any) {
  ...
  const linhasServicos = servicos.map(servico => `
    <tr>
      <td>${servico.descricao || 'N/A'}</td>
      <td>${servico.codigoTributacao || 'N/A'}</td>
      <td>${formatarItemListaServico(servico.itemListaServico) || 'N/A'}</td>
      <td class="valor">${formatarValor(servico.valorUnitario || 0)}</td>
      <td>${servico.aliquota ? `${servico.aliquota}%` : 'N/A'}</td>
      <td>${formatarData(servico.createdAt)}</td>
    </tr>
  `).join('');
...
<body>
  ...
  <table>
    <thead>
      <tr>
        <th>Descrição</th>
        <th>Código Tributação</th>
        <th>Item Lista Serviço</th>
